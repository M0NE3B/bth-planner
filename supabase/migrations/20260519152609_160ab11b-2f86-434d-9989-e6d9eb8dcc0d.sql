-- Track whether a user has been auto-migrated to the new catalog
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS catalog_migrated_at timestamp with time zone;

-- Per-user automatic sync, safe to call from the client (scoped to auth.uid()).
...
BEGIN
  ...
  SELECT program_name, catalog_migrated_at
    INTO _current_program, _already_migrated
  ...
  -- 2. Link existing user_courses to catalog via course_code
  WITH upd AS (
  ...
  -- 3. Seed missing mandatory catalog courses for the user's program
  IF _current_program IS NOT NULL THEN
    WITH ins AS (
    ...
    -- 4. Remove only truly unused orphan rows (no status, no events, no subtasks,
    -- and not part of the current program's catalog).
    WITH del AS (
